...
class helloworld:
    ...
    def __init__(self,server):
        ...
        # Registering the HELLO node
        self.server.registerNode('hello',self.name)
        
        # Registering the QUOTE node
        self.server.registerNode('quote',self.name)
        
        # Every time a client connects we send him a Hello message
        self.server.registerAction('onClientConnect',self.name)
        
        # Every time a client logs out we say good bye ...
        self.server.registerAction('onClientClose',self.name)
        
        return
    
    
    def onClientClose(self,params):
        
        client = params[0]
        client.clientSendMessage(self.server.formatMessage('hello',[],'Bye bye '+client.nickName+'. Come back soon ! '))
        
        return
    
    
    
    def onClientConnect(self,params):
        client = params[0]
        client.clientSendMessage(self.server.formatMessage('hello',[],'Hello '+client.nickName+'. Please feel free to send me the "hello" or "quote" nodes ! '))
    ...
    def doNode(self,nodeName,node,client):
        ...
        if nodeName == 'hello':
            client.clientSendMessage(self.server.formatMessage('world',[],'Hello World !'))
        elif nodeName == 'quote':
            client.clientSendMessage(self.server.formatMessage('quote',[],self.getRandomQuote()))
        return
    
    
    def getRandomQuote(self):
        
        quotes = ["The trouble with weather forecasting is that it's right too often for us to ignore it and wrong too often for us to rely on it."
                ,
